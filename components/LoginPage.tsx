...
interface Props {
  ...
  /** Android: hardware back exits login (minimize/leave app). */
...
const LoginPage: React.FC<Props> = ({ onLogin, bindAndroidBack }) => {
      ...
      <div className="z-10 w-full max-w-md space-y-12 animate-in fade-in zoom-in duration-1000">
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-[10px] uppercase tracking-[0.4em] text-[var(--text-muted)] font-bold">{t('login.tagline')}</p>
          </div>
        </div>

        <div className="bg-[var(--bg-secondary)] border border-theme rounded-[3rem] p-10 space-y-8 shadow-2xl">
          <div className="text-center space-y-2">
            <h2 className="text-xl serif text-[var(--text-primary)]">{t('login.welcome')}</h2>
            <p className="text-xs text-[var(--text-muted)]">{t('login.subtitle')}</p>
          </div>
