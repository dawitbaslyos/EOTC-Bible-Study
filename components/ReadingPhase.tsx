  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f4f1ea] text-[#2c2c2c] animate-in fade-in duration-1000 relative">
              ...
              <div key={v.verse} className="space-y-6 animate-in slide-in-from-bottom-4 duration-700">
                <div className="flex items-start gap-4 sm:gap-6">
                   ...
                   <div className="space-y-6 flex-1">
                     {v.geez && langs.geez && (
                       <p className="ethiopic text-2xl sm:text-3xl leading-[1.6] text-black/90 font-medium animate-in fade-in duration-500">{v.geez}</p>
                     )}
                     {v.text && langs.amharic && (
                       <p className="ethiopic text-2xl sm:text-3xl leading-[1.6] text-[#b4941f] font-light italic animate-in fade-in duration-500 whitespace-pre-wrap">{v.text}</p>
                     )}
                     {v.english && langs.english && (
                       <p className="text-lg sm:text-xl serif leading-relaxed text-black/75 italic font-light tracking-tight border-l-2 border-[#d4af37]/20 pl-6 animate-in fade-in duration-500">{renderEnglishText(v.english, v.commentary)}</p>
                     )}
                   </div>
                </div>
              </div>
      ...
      <div className="fixed bottom-0 left-0 w-full p-10 flex justify-center z-40 bg-gradient-to-t from-[#f4f1ea] via-[#f4f1ea]/80 to-transparent">
        ...
        >
          <span className="serif text-xl tracking-wide">{isDailyManna ? (data.chapter === 1 ? "Daily Portion" : "Summarize") : "Done Reading"}</span>
