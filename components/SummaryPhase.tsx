...
interface Props {
  /** Short closing prompt (personal prayer), not full liturgy text */
  reflectionText: string;
  onFinish: () => void;
}

const SummaryPhase: React.FC<Props> = ({ reflectionText, onFinish }) => {
    ...
    <div className="flex-1 flex flex-col p-8 bg-[var(--bg-primary)] animate-in slide-in-from-right-10 duration-700 pb-32">
      <header className="mb-12 text-center md:text-left">
        <div className="flex items-center justify-center md:justify-start space-x-3 text-[var(--gold)] mb-2 opacity-80">
          ...
          <span className="text-[10px] uppercase font-bold tracking-[0.3em]">{t('summary.dailyReflection')}</span>
        </div>
        <h1 className="text-4xl md:text-5xl serif gold-glow text-[var(--text-primary)]">{t('summary.finalThoughts')}</h1>
      </header>
      ...
      <div className="space-y-6 flex-1 max-w-2xl mx-auto w-full overflow-y-auto max-h-[60vh] custom-scrollbar mb-8 pr-2">
        <section className="bg-[var(--card-bg)] p-8 md:p-10 rounded-[2.5rem] border border-theme group hover:border-[var(--gold)]/30 transition-all shadow-xl backdrop-blur-sm relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 mb-6 relative z-10">
            ...
            <h3 className="text-[10px] uppercase font-bold tracking-[0.2em] text-[var(--text-muted)]">{t('summary.reflection')}</h3>
          </div>
      ...
      <div className="flex flex-col items-center space-y-4">
        ...
        >
          <span className="serif text-xl">{t('summary.finish')}</span>
          <Icons.ChevronRight className="w-5 h-5" />
        </button>
        <p className="text-[9px] uppercase tracking-[0.3em] text-[var(--text-muted)] font-bold">{t('summary.endSession')}</p>
      </div>
    </div>
  );
};
